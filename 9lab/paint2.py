...
class Pen(GameObject):
    ...
    def draw(self):
        for idx, point in enumerate(self.points[:-1]):
            pygame.draw.line(SCREEN,self.color,start_pos=point,end_pos=self.points[idx + 1],width=self.thickness,)
...
class RightTriangle(GameObject):
    ...
    def draw(self):
        ...
        side_len = x2-x1                      #length  
        h = math.pow(side_len**2 * 0.75, 0.5) #height
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            #Choosing event by button
            if event.type == pygame.MOUSEBUTTONDOWN:
                buttons_dict = {switch_pen:'pen',switch_square:'square',switch_triangleR :'rightT',switch_triangleW :'wrongT',switch_rhombus:'rhombus',}
                for key in buttons_dict.keys():
                    if key.rect.collidepoint(pygame.mouse.get_pos()):
                        current_shape = buttons_dict[key]
                else:
                    shapes = {'pen':Pen(start_pos=event.pos), 'square':Rectangle(start_pos=event.pos),'rightT': RightTriangle(start_pos=event.pos),'wrongT': EquilateralTriangle(start_pos=event.pos),'rhombus': Rhombus(start_pos=event.pos),}
